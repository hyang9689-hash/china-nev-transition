...
- Root report: `final_report.pdf` generated by `final_report.qmd`
- Local verification: 20/20 tests and 8/8 project-integrity checks passed.
- PDF verification: 12 pages, embedded fonts, no clipping, no blank pages, and
  no unresolved metadata or corrupt text.

The final commit, `v1.0.0` tag, Actions run, and Pages deployment remain visible
in the repository's Git and GitHub history.

## v0.2.0 publication identity
...
- Live HTML contained the canonical `hyang9689-hash/china-nev-transition` link, contained no author/GitHub placeholder, included fleet-turnover and powertrain analysis, and rendered no notebook traceback.
